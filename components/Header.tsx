...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-navy/10 bg-surface-light/90 backdrop-blur dark:border-white/10 dark:bg-surface-dark/90">
        ...
        <nav aria-label="Main" className="hidden items-center gap-1 md:flex">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-lg px-3 py-2 text-sm font-medium text-navy-700 hover:bg-navy-50 dark:text-slate-200 dark:hover:bg-white/10"
            >
              {link.label}
            </Link>
          ))}
      ...
      {open && (
        <nav id="mobile-menu" aria-label="Mobile" className="border-t border-navy/10 bg-surface-light dark:border-white/10 dark:bg-surface-dark md:hidden">
